...
class Dashboard:
    ...
    def run():
        ...
        try:
            ...
            x_test = pd.Series(response.text.split("|")[7].strip())
            y_test = pd.Series(response.text.split("|")[8].strip())
            ...
            print(f"The coeffitient is: {coef_}")
            print(f"The intercept is: {intercept_e}")
            print(f"x_test [{x_test}]")
            print(f"y_test [{y_test}]")
            print(f"y_pred [{y_pred}]")
            print(f"'he correlation of the values is: {r2score}'")
